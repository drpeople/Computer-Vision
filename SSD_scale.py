...
def run_inference(model, selected_images, images_path, scale_factor=1.0, conf_threshold=0.2):
    """
    ...
    with torch.no_grad():
        for category_id, image_files in selected_images.items():
            for image_file in image_files:
                ...
                # Get boxes, scores and labels (boxes: [x1, y1, x2, y2])
                if outputs['boxes'].numel() > 0:
                    ...
                    # Since the input image was scaled before inference,
                    # we need to divide the box coordinates by scale_factor.
                    if boxes.size > 0:
                        boxes = boxes / scale_factor
                    # Combine boxes, scores, labels into one array for consistency
...
def calculate_metrics(coco, predictions, iou_threshold=0.5):
    ...
    for category_id in coco.getCatIds():
        ...
        ap_per_class[category_id] = precision_score(category_trues, category_preds,
                                                    zero_division=0) if category_preds else 0
...
# Define the scale factors (for example: 50%, 75%, 100%, 125%, 150%, 200%, 300%)
...
csv_path = "SSD300_VGG16_scale_results.csv"
