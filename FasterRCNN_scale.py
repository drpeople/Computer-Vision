...
def scale_image(image_path, scale_factor):
    """
    Load an image from image_path and scale it by the given scale_factor.
...
def run_inference(model, selected_images, images_path, device, scale_factor=1.0,
                  conf_threshold=0.5, use_soft_nms=False, soft_nms_params=None):
    """
    Run inference on selected images with optional scaling and Soft-NMS.
    ...
    Args:
        ...
        scale_factor (float): Factor by which to scale the image (1.0 for original).
    ...
    for category_images in selected_images.values():
        for image_file in category_images:
            ...
            # Convert image to RGB and prepare tensor
...
def plot_results(results_df, metrics_to_plot):
    """Plot specified metrics versus the scale factor."""
    for metric in metrics_to_plot:
        plt.figure(figsize=(8, 6))
        plt.plot(results_df['Scale Factor'], results_df[metric], marker='o', label=metric)
        plt.xlabel('Scale Factor')
        plt.ylabel(metric)
        plt.title(f'{metric} vs. Scale Factor')
        plt.grid(True)
        plt.legend()
        plt.show()
...
def main():
    ...
    # Define the scale factors to evaluate (e.g., 50%, 75%, original, 125%, 150%, etc.)
    ...
    for scale in tqdm(scale_factors, desc="Processing scale factors"):
        print(f"\nRunning inference with scale factor = {scale}")
        predictions = run_inference(model, selected_images, VAL_IMAGES_PATH, device,
                                    scale_factor=scale, conf_threshold=0.5, use_soft_nms=use_soft_nms)
        metrics = calculate_metrics(coco, predictions)
        metrics['Scale Factor'] = scale
        results.append(metrics)
        for metric, value in metrics.items():
            if metric != 'Scale Factor':
                print(f"{metric}: {value:.4f}")

    # Save and plot results
    results_df = pd.DataFrame(results)
    csv_path = "FasterRCNN_scale_results.csv"
