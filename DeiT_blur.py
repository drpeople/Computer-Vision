import os
import torch
import timm  # Make sure to install timm via pip: pip install timm
...
# Load a pretrained DeiT model (Data-efficient image Transformer) from timm and move it to GPU
# For this example, we're using 'deit_base_patch16_224'
...
###############################################################################
def plot_metrics_vs_blur(df, save_path="DeiT_metrics_vs_blur.png"):
...
def main():
    ...
    csv_path = "DeiT_blur_results.csv"  # Updated filename for DeiT usage
    df.to_csv(csv_path, index=False)
    print(f"Saved metrics for all blur levels to: {csv_path}")

    plot_metrics_vs_blur(df, save_path="DeiT_metrics_vs_blur.png")
