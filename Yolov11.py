...
#                 image = cv2.imread(image_path)
#
#             # Run model inference for yolov11
...
# Save results to CSV
results_df = pd.DataFrame(results)
csv_path = "Yolo_scale11_results.csv"
results_df.to_csv(csv_path, index=False)
print(f"\nResults saved to {csv_path}")
...
#                 results[0].boxes.data.cpu().numpy() if results[0].boxes else np.empty((0, 6))
#             )
#     return predictions
#
# # Helper function to compute IoU
...
# # Updated noise levels: normalized values 0.0, 0.05, 0.1, 0.2, 0.3
...
# # Save results to a CSV file
# results_df = pd.DataFrame(results)
# csv_path = "Yolo11_noise_results.csv"
# results_df.to_csv(csv_path, index=False)
# print(f"\nResults saved to {csv_path}")
#
# # Plotting results
