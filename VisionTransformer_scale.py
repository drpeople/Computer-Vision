...
# Import ViT‑B‑16 weights instead of DenseNet121 or Swin‑B weights
...
# Load a pretrained ViT‑B‑16 model using its recommended weights and move it to GPU
...
# Use default normalization values from ViT‑B‑16 weights metadata (or fallback to defaults)
...
###############################################################################
def plot_metrics_vs_scale(df, save_path="ViT_metrics_vs_scale.png"):
...
def main():
    ...
    for scale_factor in scale_levels:
        ...
        # then resizes and center crops to match the input size expected by ViT‑B‑16.
    ...
    csv_path = "ViT_scale_results.csv"  # Updated filename for ViT usage
    df.to_csv(csv_path, index=False)
    print(f"Saved metrics for all scale factors to: {csv_path}")

    plot_metrics_vs_scale(df, save_path="ViT_metrics_vs_scale.png")
