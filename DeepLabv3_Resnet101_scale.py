...
# ---------------------------
# 3) CUSTOM SCALING TRANSFORM FOR SEGMENTATION
# ---------------------------
...
class SegmentationScaleTransform:
    ...
    (Note: No additional fixed-size resize is applied—images will have variable sizes
...
def run_inference(dataset, batch_size=1):
    ...
    Returns predicted masks and ground-truth masks for metric computation.

    Note: batch_size is set to 1 because the images (after scaling) have variable sizes.
    Also, num_workers is set to 0 and pin_memory is disabled to reduce memory overhead.
    """
    loader = DataLoader(
        dataset,
        ...
        pin_memory=False  # disable pinned memory to avoid extra memory overhead
    )
...
def main():
    ...
    for scale in scale_levels:
        ...
        transform = SegmentationScaleTransform(scale_factor=scale)
    ...
    csv_path = "deepLabv3_scale_results.csv"
