...
# Import RegNet_Y_400MF weights instead of ConvNeXt weights
...
# Load pretrained RegNet_Y_400MF model using its recommended weights and move it to GPU
weights = RegNet_Y_400MF_Weights.IMAGENET1K_V1
model = models.regnet_y_400mf(weights=weights).to(device)
model.eval()
...
###############################################################################
def plot_metrics_vs_scale(df, save_path="RegNet_metrics_vs_scale.png"):
...
def main():
    ...
    for scale_factor in scale_levels:
        ...
        # then resizes and center crops to match the input size expected by the model.
    ...
    csv_path = "RegNet_scale_results.csv"  # Updated filename for RegNet usage
    df.to_csv(csv_path, index=False)
    print(f"Saved metrics for all scale factors to: {csv_path}")

    plot_metrics_vs_scale(df, save_path="RegNet_metrics_vs_scale.png")
