...
# Helper function to preprocess an image for EfficientDet inference
def preprocess_image_for_inference(image, image_size):
    ...
    Returns the tensor and scale factors for mapping boxes back.
    """
    ...
    # Compute scale factors to map predicted boxes back to the rotated image size
    scale_x = original_w / image_size
    scale_y = original_h / image_size
    return tensor, (scale_x, scale_y)


# Run inference and collect predictions using EfficientDet for rotated images
def run_inference(model, selected_images, images_path, angle=0, conf_threshold=0.25, image_size=512, device='cuda'):
    predictions = {}
    for category_id, image_files in selected_images.items():
        for image_file in image_files:
            ...
            # Preprocess the rotated image for EfficientDet
            input_tensor, (scale_x, scale_y) = preprocess_image_for_inference(rotated_image, image_size)
            ...
            if outputs.shape[0] > 0:
                # Scale predicted boxes back to the rotated image dimensions
...
def calculate_metrics(coco, predictions, iou_threshold=0.5):
    ...
    for image_file, data in predictions.items():
        ...
        # Convert ground truth bboxes from [x, y, w, h] to [x1, y1, x2, y2]
    ...
    # Compute per-category average precision using scikit-learn
    for category_id in coco.getCatIds():
...
# Perform inference for rotated images at different angles
rotation_angles = [0, 30, 60, 90, 120, 150, 180]
results = []

for angle in rotation_angles:
    print(f"\nInference with rotation angle {angle}")
    predictions = run_inference(model, selected_images, val_images_path, angle=angle, conf_threshold=0.25,
...
csv_path = "EfficientDet_rotation_results.csv"
