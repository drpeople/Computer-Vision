...
# Import ConvNeXt Base weights instead of MobileNet weights
...
###############################################################################
# CUSTOM TRANSFORM CLASS: SCALING
###############################################################################
class ScaleTransform:
    ...
    interpolation: interpolation method for resizing (default: bicubic).
    """
...
# Load pretrained ConvNeXt Base model using its recommended weights and move it to GPU
weights = ConvNeXt_Base_Weights.IMAGENET1K_V1
model = models.convnext_base(weights=weights).to(device)
model.eval()
...
# Use default normalization values if not present in weights.meta
    ...
    def __init__(self, base_folder, transform):
        ...
        for subfolder in os.listdir(base_folder):
            ...
            if os.path.isdir(subfolder_path):
                # The subfolder name is used as the true label (as an integer)
...
###############################################################################
# PLOTTING: VISUALIZATION OF METRICS VS. SCALE FACTOR
###############################################################################
def plot_metrics_vs_scale(df, save_path="ConvNeXt_metrics_vs_scale.png"):
    ...
    plt.title("Model Performance vs. Scale Factor")
...
def main():
    ...
    for scale_factor in scale_levels:
        print("==================================================")
        print(f"Running inference with scale_factor={scale_factor}")
        print("==================================================")

        # Define a transform pipeline that first scales the image,
        # then resizes and center crops to match the input size expected by ConvNeXt.
        transform_with_scale = transforms.Compose([
            ScaleTransform(scale_factor=scale_factor, interpolation=Image.BICUBIC),
        ...
        print(f"\n--- Metrics for scale_factor={scale_factor:.2f} ---")
    ...
    csv_path = "ConvNeXt_scale_results.csv"  # Updated filename for ConvNeXt usage
    df.to_csv(csv_path, index=False)
    print(f"Saved metrics for all scale factors to: {csv_path}")

    plot_metrics_vs_scale(df, save_path="ConvNeXt_metrics_vs_scale.png")
    print("\nAll scale levels processed. End of script.")
