...
# Import Swin-B weights instead of DenseNet121 weights
...
# Load a pretrained Swin-B model using its recommended weights and move it to GPU
...
# Use default normalization values from Swin-B weights metadata (or fallback to defaults)
...
###############################################################################
def plot_metrics_vs_scale(df, save_path="SwinB_metrics_vs_scale.png"):
...
def main():
    ...
    for scale_factor in scale_levels:
        ...
        # then resizes and center crops to match the input size expected by Swin-B.
    ...
    csv_path = "SwinB_scale_results.csv"  # Updated filename for Swin-B usage
    df.to_csv(csv_path, index=False)
    print(f"Saved metrics for all scale factors to: {csv_path}")

    plot_metrics_vs_scale(df, save_path="SwinB_metrics_vs_scale.png")
