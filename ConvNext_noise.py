...
# Import ConvNeXt Base weights instead of MobileNetV2 weights
...
class GaussianNoiseTransform:
    ...
    def __call__(self, img):
        # Convert PIL image to numpy array as float32
        np_img = np.array(img, dtype=np.float32)
        # Scale image to [0, 1]
        np_img = np_img / 255.0
        # Generate Gaussian noise with mean 0 and specified std (in normalized domain)
...
# Load a pretrained ConvNeXt Base model using its recommended weights and move it to GPU
...
###############################################################################
def plot_metrics_vs_noise(df, save_path="ConvNeXt_metrics_vs_noise.png"):
...
###############################################################################
def main():
    # Use normalized noise levels (similar to the YOLO9 noise script)
    ...
    csv_path = "ConvNeXt_noise_results.csv"  # Updated filename to reflect ConvNeXt usage
