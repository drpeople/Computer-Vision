...
class LeafSegFineTuneDataset(Dataset):
    def __init__(self, images_dir, masks_dir, image_transform, image_size=IMAGE_SIZE, noise_std=0.0):
    ...
    def __getitem__(self, idx):
        ...
        if self.noise_std > 0:
            image = add_noise(image, self.noise_std)
...
def add_noise(image, noise_std):
    ...
    if noise_std > 0:
        ...
        noise = torch.randn(tensor_img.size()) * noise_std
...
def main():
    ...
    for noise_std in noise_levels:
        ...
        dataset = LeafSegFineTuneDataset(test_images_dir, test_masks_dir, image_transform, image_size=IMAGE_SIZE, noise_std=noise_std)
    ...
    results_df.to_csv("pspnet_noise_results.csv", index=False)
    print("CSV saved: pspnet_noise_results.csv")
