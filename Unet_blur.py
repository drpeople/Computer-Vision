...
class LeafSegFineTuneDataset(Dataset):
    ...
    def __init__(self, images_dir, masks_dir, image_size=(544, 544), sigma=0):
...
def run_inference(dataset, batch_size=4):
    ...
    for batch in loader:
        ...
        with autocast():
            ...
            pred_maps = (preds > 0.5).long().squeeze(1)  # shape: (B, H, W)
...
# 6) PLOTTING RESULTS
# ---------------------------
def plot_blur_sweep(blur_levels, pixel_accs, mious, save_path="unet_blur_results.png"):
    plt.figure(figsize=(8, 6))
    plt.plot(blur_levels, pixel_accs, marker='o', label='Pixel Accuracy')
    plt.plot(blur_levels, mious, marker='s', label='Mean IoU')
    plt.title("U-Net Segmentation vs. Gaussian Blur Sigma")
...
def main():
    ...
    for sigma in blur_levels:
        ...
        dataset = LeafSegFineTuneDataset(test_images_dir, test_masks_dir, image_size=(544, 544), sigma=sigma)
    ...
    results_df.to_csv("unet_blur_results.csv", index=False)
    print("CSV saved: unet_blur_results.csv")
