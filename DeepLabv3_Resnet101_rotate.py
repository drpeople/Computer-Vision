...
class SegmentationTransform:
    ...
    2. Resizes them to a fixed size.
    3. Creates a 'valid region' map to detect new black corners and sets those pixels to 255 (ignore) in the final mask.
    4. Converts the image to tensor & normalizes.
    ...
    def __call__(self, image, mask):
        ...
        # 2) Create a valid region map (white image) and rotate it.
        w, h = image.size
        valid_map = Image.new("L", (w, h), color=1)  # all ones
        valid_map_rot = valid_map.rotate(self.angle, resample=Image.NEAREST, expand=True)
        ...
        # 4) Convert mask to tensor (NumPy array) for processing.
        mask_out = torch.from_numpy(np.array(mask_out, dtype=np.int64))

        valid_np = np.array(valid_map_resized, dtype=np.uint8)  # 1 where original, 0 where new/black

        # 5) Set new black corners (where valid_np==0) to ignore label (255).
        mask_np = mask_out.numpy()
        mask_np[valid_np == 0] = 255
        mask_out = torch.from_numpy(mask_np)
...
def build_transform(angle=0, resize=(520, 520)):
    ...
    and marks newly introduced black corners as ignore (255) in the mask.
    """
    return SegmentationTransform(angle=angle, resize=resize)
...
# ---------------------------
def main():
    # Run baseline inference on original images (angle=0) as reference.
    ...
    baseline_preds, _ = run_inference(dataset, batch_size=8)
    ...
    for angle in angles:
        ...
        # Inverse rotate predictions (if angle is nonzero) to align with baseline.
        adjusted_preds = []
        for pred in preds:
            if angle != 0:
        ...
        # Compute metrics comparing adjusted predictions with baseline predictions.
