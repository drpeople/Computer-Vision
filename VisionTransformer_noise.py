...
# Import ViT‑B‑16 weights instead of Swin‑B weights
...
###############################################################################
# CUSTOM TRANSFORM CLASS: GAUSSIAN NOISE (Normalized)
###############################################################################
class GaussianNoiseTransform:
    def __init__(self, noise_std=0.0):
        # noise_std is provided in the normalized domain (e.g., 0.0, 0.05, 0.1, etc.)
        self.noise_std = noise_std

    def __call__(self, img):
        # Convert PIL image to NumPy array as float32 and scale to [0, 1]
        np_img = np.array(img, dtype=np.float32) / 255.0
        # Generate Gaussian noise with the normalized standard deviation
...
###############################################################################
def plot_metrics_vs_noise(df, save_path="ViT_metrics_vs_noise.png"):
...
def main():
    ...
    csv_path = "ViT_noise_results.csv"  # Updated filename for ViT usage
