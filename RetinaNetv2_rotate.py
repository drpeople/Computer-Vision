...
# from pycocotools.cocoeval import COCOeval  # <-- added for proper mAP
...
#
#     # Rotate and re-compute annotations
#     rotated_annotations = []
#     for ann in annotations:
#         x, y, bw, bh = ann['bbox']
#         box = np.array([[x, y], [x + bw, y], [x, y + bh], [x + bw, y + bh]], dtype=np.float32)
#         box = box.reshape(-1, 1, 2)
#         rotated_box = cv2.transform(box, rotation_matrix).reshape(-1, 2)
...
#     Returns a dict mapping file names to predictions and ground truth.
#     """
#     predictions = {}
#     for _, image_files in selected_images.items():
#         for image_file in image_files:
#             image_path = os.path.join(images_path, image_file)
#
#             # Load GT and rotate
#             img_id = next(img['id'] for img in coco.dataset['images'] if img['file_name'] == image_file)
...
#             pred_data = np.column_stack((boxes[keep], scores[keep], labels[keep])) if keep.any() else np.empty((0,6))
#
...
#     """
#     Calculate detection metrics including global Precision, Recall,
#     F1 Score, Mean IoU, and COCO mAP (IoU=0.50:0.95).
#     """
#     y_true, y_pred, iou_scores = [], [], []
#
#     # Build list for COCOeval
#     coco_results = []
#
#     for file_name, data in predictions.items():
#         image_id = data['image_id']
#         preds = data['predictions']
#         gt_boxes = [[x, y, x + w, y + h] for x, y, w, h in [ann['bbox'] for ann in data['ground_truth']]]
#
#         # Match preds to GT for global metrics
#         matched = set()
#         for pb in preds[:, :4] if preds.size else []:
#             best_iou, best_idx = 0, -1
#             for i, tb in enumerate(gt_boxes):
...
#             })
#
#     # Compute global precision/recall/F1/Mean IoU
...
#         coco_eval.summarize()
#         mAP = float(coco_eval.stats[0])  # IoU=0.50:0.95
#     else:
#         mAP = 0.0
#
#     return {
...
#     print(" ".join([f"{k}: {v:.4f}" for k,v in metrics.items() if k != 'Rotation Angle']))
#
# # Save results & plot
# results_df = pd.DataFrame(results)
# results_df.to_csv("RetinaNet_v2_rotation_results.csv", index=False)
# print("Results saved to RetinaNet_v2_rotation_results.csv")
#
# for metric in ['Precision','Recall','F1 Score','Mean IoU','mAP']:
#     plt.figure(figsize=(8,6))
#     plt.plot(results_df['Rotation Angle'], results_df[metric], marker='o')
...
# Save results to CSV
results_df = pd.DataFrame(results)
csv_path = "RetinaNetv2_rotation_results.csv"
