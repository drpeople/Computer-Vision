...
class LeafSegFineTuneDataset(Dataset):
    def __init__(self, images_dir, masks_dir, processor, image_size=(512, 512), noise_std=0.0):
...
# 5) PLOTTING RESULTS
# ---------------------------
def plot_noise_sweep(noise_levels, pixel_accs, mious, save_path="segformer_noise_results.png"):
    plt.figure(figsize=(8, 6))
    plt.plot(noise_levels, pixel_accs, marker='o', label='Pixel Accuracy')
    plt.plot(noise_levels, mious, marker='s', label='Mean IoU')
    plt.title("SegFormer Segmentation vs. Gaussian Noise Std")
...
def main():
    ...
    for noise_std in noise_levels:
        ...
        dataset = LeafSegFineTuneDataset(test_images_dir, test_masks_dir, processor, image_size=(512, 512),
                                         noise_std=noise_std)
    ...
    results_df.to_csv("segformer_noise_results.csv", index=False)
    print("CSV saved: segformer_noise_results.csv")
